...
import generator, translation_quality_model, helper, discriminator
...
def train_discriminator():
    ...
    # pos_val, neg_val = helper.generate_samples(h5f, dev_minibatches, vocab, label_voc, gen, args)
    # val_inp, val_target = helper.prepar_discriminator_data(pos_val, neg_val)
        ...
        for epoch in range(args.dis_epochs):
            ...
            for b_idx in range(args.dis_train_batches):
                ...
                if num_batch % args.save_freq == 0:
                    ...
                    #             'config_args': args}, 'models/discriminator_gru_adagrad.pt')
                    num_batch = 0
                    total_loss = 0.0
                    stime = time.time()
    # dis_log.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dis_dev_batches', type=int, default=2,
    ...
    parser.add_argument('--d_steps', type=int, default=10000,
    ...
    parser.add_argument('--dis_hidden_dim', type=int, default=300,
    ...
    # dis = discriminator.Discriminator(args.dis_embedding_dim, args.dis_hidden_dim, len_voc).cuda()
    We_vocab = get_wordmap(vocab, args)
    dis = translation_quality_model.translation_quality_model(args.dis_hidden_dim, args.dis_embedding_dim, len_voc, We_vocab).cuda()
