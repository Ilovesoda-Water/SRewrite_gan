...
def indexify_transformations_no_EOP(in_p, out_p, label_voc, args):
    ...
    for idx in range(len(in_p)):
        ...
        in_full_trans = (deleaf(in_tree))[:-1]
        out_full_trans = (deleaf(out_tree))[:-1]
...
def parse_tree_level(tree, treerate, level=None):
    def parse_tree_level_dropout2(tree, level, mlevel):
        if level == mlevel:
            for idx, n in enumerate(tree):
...
def get_wordmap(vocab, args, embedding_dim=300):
    words = {}
    We = []
    f = io.open(args.paragram_sl999, 'r', encoding='utf-8')
    lines = f.readlines()
    ...
    for (n,i) in enumerate(lines):
        ...
        ct += 1
        We.append(v)
    We_vocab = [0] * len(vocab)
    for word in vocab:
        if word in words:
            We_vocab[vocab[word]] = We[words[word]]
        else:
            We_vocab[vocab[word]] = [float(i) for i in np.random.normal(scale=0.1, size=embedding_dim).astype(np.float32)]
    We_vocab[vocab['PAD']] = [0.] * embedding_dim
    return  We_vocab
